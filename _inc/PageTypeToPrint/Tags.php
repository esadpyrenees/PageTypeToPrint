<?php
  /* 
    This file defines functions needed by PageTypeToPrint.
    - slugify and remove_accent helps to build clean `id` attributes for elements
    - pttp_log is a simple, even naive, log-to-file utility
    - parse_shortcode and process_shortcodes help to deal with custyom shortcodes that one can type in markdown sources
    - video, youtube, vimeo are used to parse shortcodes like `(video: http://video.url)` and automate the build of a webcomponent containing an iframe
  */

  use Kaoken\MarkdownIt\MarkdownIt;

  /**
   * Slugify string (removing accents, special chars, etc.)
   *
   * @param string $str 
   * @return string 
   */
  ...
  /**
   * Quick n dirty log-to-file util
   *
   * @param string $msg Message to be written in file.
   * @return void 
   */

  function pttp_log($msg) {
    $log_dirname = "log";
   ...
   * @return array Associative array `$shortcode => $string, $html => string`.
   */
   ...
   * Returns array()
   * [
   *   "md"=> string: type of shorcode, 
   *   "figures" => array: optional, html for figures (in case of figures found within the markdown input)
   * ] 
   ...
   * Only for Youtube and Vimeo
   *
   * @param string $url Url of the video.
   * @return string webcomponent HTML.
   */
